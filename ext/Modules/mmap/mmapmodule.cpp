...
static int my_getallocationgranularity(void) {
  SYSTEM_INFO si;
  GetSystemInfo(&si);
...
static int my_getpagesize(void) { return sysconf(_SC_PAGESIZE); }
...
#ifdef MS_WINDOWS
  if (m_obj->data != NULL) UnmapViewOfFile(m_obj->data);
  if (m_obj->map_handle != NULL) CloseHandle(m_obj->map_handle);
  if (m_obj->file_handle != INVALID_HANDLE_VALUE)
    CloseHandle(m_obj->file_handle);
  if (m_obj->tagname) PyMem_Free(m_obj->tagname);
#endif /* MS_WINDOWS */

#ifdef UNIX
  if (m_obj->fd >= 0) (void)close(m_obj->fd);
  if (m_obj->data != NULL) {
    munmap(m_obj->data, m_obj->size);
  }
#endif /* UNIX */

  if (m_obj->weakreflist != NULL) PyObject_ClearWeakRefs((PyObject*)m_obj);
  PyObject_Del(m_obj);
  Py_DECREF(tp);
}
...
#ifdef UNIX
  if (0 <= self->fd) (void)close(self->fd);
  self->fd = -1;
  if (self->data != NULL) {
...
static PyObject* mmap_read_line_method(mmap_object* self,
  ...
  if (!remaining) return PyBytes_FromString("");
...
static int mmap_convert_ssize_t(PyObject* obj, void* result) {
  Py_ssize_t limit;
  if (obj == Py_None) {
    limit = -1;
    ...
    PyErr_Format(PyExc_TypeError, "integer argument expected, got '%.200s'",
...
static PyObject* mmap_read_method(mmap_object* self, PyObject* args) {
  ...
  remaining = (self->pos < self->size) ? self->size - self->pos : 0;
  if (num_bytes < 0 || num_bytes > remaining) num_bytes = remaining;
...
static PyObject* mmap_gfind(mmap_object* self, PyObject* args, int reverse) {
  ...
  CHECK_VALID(NULL);
  if (!PyArg_ParseTuple(args, reverse ? "y*|nn:rfind" : "y*|nn:find", &view,
                        &start, &end)) {
    return NULL;
  } else {
    const char *p, *start_p, *end_p;
    int sign = reverse ? -1 : 1;
    const char* needle = (char*)view.buf;
    ...
    for (p = (reverse ? end_p - len : start_p);
      ...
      Py_ssize_t i;
      for (i = 0; i < len && needle[i] == p[i]; ++i) /* nothing */
        ;
      if (i == len) {
...
static PyObject* mmap_find_method(mmap_object* self, PyObject* args) {
  return mmap_gfind(self, args, 0);
}

static PyObject* mmap_rfind_method(mmap_object* self, PyObject* args) {
  return mmap_gfind(self, args, 1);
}

static int is_writable(mmap_object* self) {
  if (self->access != ACCESS_READ) return 1;
...
static PyObject* mmap_write_method(mmap_object* self, PyObject* args) {
  Py_buffer data;

  CHECK_VALID(NULL);
  if (!PyArg_ParseTuple(args, "y*:write", &data)) return (NULL);
...
static PyObject* mmap_write_byte_method(mmap_object* self, PyObject* args) {
  char value;

  CHECK_VALID(NULL);
  if (!PyArg_ParseTuple(args, "b:write_byte", &value)) return (NULL);

  if (!is_writable(self)) return NULL;
...
static PyObject* mmap_size_method(mmap_object* self,
  ...
  if (self->file_handle != INVALID_HANDLE_VALUE) {
    ...
    if (low == INVALID_FILE_SIZE) {
      ...
      if (error != NO_ERROR) return PyErr_SetFromWindowsErr(error);
    }
    if (!high && low < LONG_MAX) return PyLong_FromLong((long)low);
...
#ifdef UNIX
  {
    ...
    if (_Py_fstat(self->fd, &status) == -1) return NULL;
...
static PyObject* mmap_resize_method(mmap_object* self, PyObject* args) {
  Py_ssize_t new_size;
  CHECK_VALID(NULL);
  if (!PyArg_ParseTuple(args, "n:resize", &new_size) || !is_resizeable(self)) {
    return NULL;
  }
...

  {
    ...
    SetFilePointer(self->file_handle, newSizeLow, &newSizeHigh, FILE_BEGIN);
    /* Change the size of the file */
    SetEndOfFile(self->file_handle);
    /* Create another mapping object and remap the file view */
    self->map_handle = CreateFileMapping(self->file_handle, NULL,
                                         PAGE_READWRITE, 0, 0, self->tagname);
    if (self->map_handle != NULL) {
      self->data = (char*)MapViewOfFile(self->map_handle, FILE_MAP_WRITE,
                                        off_hi, off_lo, new_size);
      if (self->data != NULL) {
        self->size = new_size;
        Py_INCREF(Py_None);
        return Py_None;
...
static PyObject* mmap_flush_method(mmap_object* self, PyObject* args) {
  Py_ssize_t offset = 0;
  Py_ssize_t size = self->size;
  CHECK_VALID(NULL);
  if (!PyArg_ParseTuple(args, "|nn:flush", &offset, &size)) return NULL;
...
static PyObject* mmap_seek_method(mmap_object* self, PyObject* args) {
  Py_ssize_t dist;
  int how = 0;
  CHECK_VALID(NULL);
  if (!PyArg_ParseTuple(args, "n|i:seek", &dist, &how))
    ...
    switch (how) {
      ...
      case 1: /* relative to current position */
        if (PY_SSIZE_T_MAX - self->pos < dist) goto onoutofrange;
        where = self->pos + dist;
        break;
      case 2: /* relative to end */
        if (PY_SSIZE_T_MAX - self->size < dist) goto onoutofrange;
    ...
    if (where > self->size || where < 0) goto onoutofrange;
    self->pos = where;
    Py_INCREF(Py_None);
    return Py_None;
  }
...
static PyObject* mmap_move_method(mmap_object* self, PyObject* args) {
  ...
  if (!PyArg_ParseTuple(args, "nnn:move", &dest, &src, &cnt) ||
    ...
    if (dest < 0 || src < 0 || cnt < 0) goto bounds;
    if (self->size - dest < cnt || self->size - src < cnt) goto bounds;
...
static PyObject* mmap_closed_get(mmap_object* self, void* Py_UNUSED(ignored)) {
#ifdef MS_WINDOWS
...
static PyObject* mmap__exit__method(PyObject* self, PyObject* Py_UNUSED(args)) {
  return PyObject_CallMethod(self, "close", NULL);
}

#ifdef MS_WINDOWS
static PyObject* mmap__sizeof__method(mmap_object* self,
                                      void* Py_UNUSED(unused)) {
  Py_ssize_t res;

  res = _PyObject_SIZE(Py_TYPE(self));
  if (self->tagname) res += strlen(self->tagname) + 1;
...
static PyGetSetDef mmap_object_getset[] = {
    {(char*)"closed", (getter)mmap_closed_get, NULL, NULL}, {NULL}};
...
static PyObject* mmap_item(mmap_object* self, Py_ssize_t i) {
  CHECK_VALID(NULL);
  if (i < 0 || i >= self->size) {
...
static PyObject* mmap_subscript(mmap_object* self, PyObject* item) {
  CHECK_VALID(NULL);
  if (PyIndex_Check(item)) {
    ...
    if (slicelen <= 0)
      ...
      return PyBytes_FromStringAndSize(self->data + start, slicelen);
    else {
    ...
    PyErr_SetString(PyExc_TypeError, "mmap indices must be integers");
    return NULL;
  }
}
...
static int mmap_ass_item(mmap_object* self, Py_ssize_t i, PyObject* v) {
  ...
  if (!is_writable(self)) return -1;
  buf = PyBytes_AsString(v);
  self->data[i] = buf[0];
  return 0;
}

static int mmap_ass_subscript(mmap_object* self, PyObject* item,
                              PyObject* value) {
  CHECK_VALID(-1);
  ...
  if (PyIndex_Check(item)) {
    ...
    if (value == NULL) {
      PyErr_SetString(PyExc_TypeError, "mmap doesn't support item deletion");
      return -1;
    }
    if (!PyIndex_Check(value)) {
      PyErr_SetString(PyExc_TypeError, "mmap item value must be an int");
      return -1;
    }
    ...
    if (PyObject_GetBuffer(value, &vbuf, PyBUF_SIMPLE) < 0) return -1;
    if (vbuf.len != slicelen) {
      PyErr_SetString(PyExc_IndexError, "mmap slice assignment is wrong size");
    ...
    PyErr_SetString(PyExc_TypeError, "mmap indices must be integer");
    return -1;
  }
}

static PyObject* new_mmap_object(PyTypeObject* type, PyObject* args,
                                 PyObject* kwdict);
...
static PyMemberDef mmap_members[] = {
    {(char*)"__weaklistoffset__", T_INT, offsetof(mmap_object, weakreflist)},
    {NULL}};

static PyType_Slot mmap_object_type_slots[] = {
...
static PyType_Spec mmap_object_type_spec = {
    "mmap.mmap", sizeof(mmap_object), 0,
    Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE, mmap_object_type_slots};
...
static PyObject* new_mmap_object(PyTypeObject* type, PyObject* args,
  ...
  static char* keywords[] = {(char*)"fileno",
                             (char*)"length",
                             (char*)"flags",
                             (char*)"prot",
                             (char*)"access",
  ...
  switch ((access_mode)access) {
    ...
    case ACCESS_DEFAULT:
      ...
      break;
    default:
      return PyErr_Format(PyExc_ValueError, "mmap invalid access parameter.");
  }

#ifdef __APPLE__
  /* Issue #11277: fsync(2) is not enough on OS X - a special, OS X specific
     fcntl(2) is necessary to force DISKSYNC and get around mmap(2) bug */
  if (fd != -1) (void)fcntl(fd, F_FULLFSYNC);
#endif

  if (fd != -1) {
    Py_BEGIN_ALLOW_THREADS fstat_result = _Py_fstat_noraise(fd, &status);
  ...
  if (fd != -1 && fstat_result == 0 && S_ISREG(status.st_mode)) {
    if (map_size == 0) {
      if (status.st_size == 0) {
        PyErr_SetString(PyExc_ValueError, "cannot mmap an empty file");
        return NULL;
      }
      ...
      if (status.st_size - offset > PY_SSIZE_T_MAX) {
        PyErr_SetString(PyExc_ValueError, "mmap length is too large");
        return NULL;
      }
  ...
  void* alloc_func = PyType_GetSlot(type, Py_tp_alloc);
  m_obj = (mmap_object*)reinterpret_cast<allocfunc>(alloc_func)(type, 0);
  if (m_obj == NULL) {
    return NULL;
  }
  m_obj->data = NULL;
  ...
  m_obj->data = (char*)mmap(NULL, map_size, prot, flags, fd, offset);
...
static PyObject* new_mmap_object(PyTypeObject* type, PyObject* args,
  ...
  static char* keywords[] = {"fileno", "length", "tagname",
                             "access", "offset", NULL};

  if (!PyArg_ParseTupleAndKeywords(args, kwdict, "in|ziL", keywords, &fileno,
                                   &map_size, &tagname, &access, &offset)) {
    return NULL;
  }
  ...
  if (fileno != -1 && fileno != 0) {
    ...
    _Py_BEGIN_SUPPRESS_IPH fh = (HANDLE)_get_osfhandle(fileno);
    _Py_END_SUPPRESS_IPH if (fh == (HANDLE)-1) {
      PyErr_SetFromErrno(PyExc_OSError);
      return NULL;
    }
  ...
  m_obj->map_handle = CreateFileMapping(m_obj->file_handle, NULL, flProtect,
                                        size_hi, size_lo, m_obj->tagname);
  if (m_obj->map_handle != NULL) {
    m_obj->data = (char*)MapViewOfFile(m_obj->map_handle, dwDesiredAccess,
                                       off_hi, off_lo, m_obj->size);
    if (m_obj->data != NULL)
      return (PyObject*)m_obj;
    else {
      dwErr = GetLastError();
...
static void setint(PyObject* m, const char* name, long value) {
  PyObject* o = PyLong_FromLong(value);
  if (o == NULL || PyModule_AddObject(m, name, o) == 0) return;
  Py_DECREF(o);
}

static struct PyModuleDef mmapmodule = {
    PyModuleDef_HEAD_INIT, "mmap", NULL, -1, NULL, NULL, NULL, NULL, NULL,
};

PyMODINIT_FUNC PyInit_mmap(void) {
  PyObject *module, *mmap_object_type;

  module = PyState_FindModule(&mmapmodule);
  if (module != NULL) {
    Py_INCREF(module);
    return module;
  }
  ...
  mmap_object_type = PyType_FromSpec(&mmap_object_type_spec);
  Py_INCREF(mmap_object_type);
  PyModule_AddObject(module, "mmap", mmap_object_type);
  Py_INCREF(PyExc_OSError);
  PyModule_AddObject(module, "error", PyExc_OSError);
  ...
  PyState_AddModule(module, &mmapmodule);
  return module;
}
