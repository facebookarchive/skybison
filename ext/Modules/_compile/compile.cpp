                ...
                int optimize, PyArena *arena)
{
    ...
    if (filename == NULL)
        return NULL;
    co = compile::PyAST_CompileObject(mod, filename, flags, optimize, arena);
    Py_DECREF(filename);
    return co;
...
PyNode_Compile(struct _node *n, const char *filename)
{
    ...
    if (mod)
        co = compile::PyAST_Compile(mod, filename, NULL, arena);
    PyArena_Free(arena);
    return co;
}
...
static int
compiler_next_instr(struct compiler *, basicblock *b)
{
    assert(b != NULL);
    if (b->b_instr == NULL) {
...
static Py_ssize_t
compiler_add_o(struct compiler *, PyObject *dict, PyObject *o)
{
    PyObject *t, *v;
    Py_ssize_t arg;
                              ...
                              asdl_seq *kw_defaults)
{
    ...
    for (i = 0; i < asdl_seq_LEN(kwonlyargs); i++) {
        arg_ty arg = static_cast<arg_ty>(asdl_seq_GET(kwonlyargs, i));
        expr_ty default_ = static_cast<expr_ty>(asdl_seq_GET(kw_defaults, i));
        if (default_) {
...
{
    basicblock *try_block, *except, *end, *after_try, *try_cleanup,
               *after_loop_else;
    ...
    try_block = compiler_new_block(c);
    ...
    if (try_block == NULL || except == NULL || end == NULL
            || after_try == NULL || try_cleanup == NULL
            || after_loop_else == NULL)
        return 0;

    ADDOP_JREL(c, SETUP_LOOP, end);
    if (!compiler_push_fblock(c, LOOP, try_block))
        return 0;

    VISIT(c, expr, s->v.AsyncFor.iter);
    ADDOP(c, GET_AITER);

    compiler_use_next_block(c, try_block);


    ADDOP_JREL(c, SETUP_EXCEPT, except);
    if (!compiler_push_fblock(c, EXCEPT, try_block))
        return 0;
    ...
    ADDOP(c, POP_BLOCK);
    compiler_pop_fblock(c, EXCEPT, try_block);
    ...
    ADDOP_JABS(c, JUMP_ABSOLUTE, try_block);
    ...
    compiler_pop_fblock(c, LOOP, try_block);
...
static int
binop(struct compiler *, operator_ty op)
{
    switch (op) {
    case Add:
        return BINARY_ADD;
...
static int
inplace_binop(struct compiler *, operator_ty op)
{
    switch (op) {
    case Add:
        return INPLACE_ADD;
                  ...
                  int single_op, int inner_op, int outer_op)
{
    ...
    for (i = 0; i < n; i++) {
        expr_ty elt = static_cast<expr_ty>(asdl_seq_GET(elts, i));
...
maybe_optimize_method_call(struct compiler *c, expr_ty e)
{
    ...
    for (i = 0; i < argsl; i++) {
        expr_ty elt = static_cast<expr_ty>(asdl_seq_GET(args, i));
        if (elt->kind == Starred_kind) {
            return -1;
        }
    }
...
compiler_subkwargs(struct compiler *c, asdl_seq *keywords, Py_ssize_t begin, Py_ssize_t end)
{
    ...
    if (n > 1) {
        for (i = begin; i < end; i++) {
            kw = static_cast<keyword_ty>(asdl_seq_GET(keywords, i));
                     ...
                     asdl_seq *keywords)
{
    ...
    for (i = 0; i < nkwelts; i++) {
        keyword_ty kw = static_cast<keyword_ty>(asdl_seq_GET(keywords, i));
        if (kw->arg == NULL) {
            mustdictunpack = 1;
            break;
        }
...
{
    comprehension_ty gen;
    basicblock *if_cleanup, *try_block,
...
compiler_async_with(struct compiler *c, stmt_ty s, int pos)
{
    ...
    withitem_ty item = static_cast<withitem_ty>(asdl_seq_GET(s->v.AsyncWith.items, pos));
...
{
    basicblock *block, *finally;
    withitem_ty item = static_cast<withitem_ty>(asdl_seq_GET(s->v.With.items, pos));
...
optimize_str_mod(struct compiler *c, expr_ty e)
{
    ...
    if (rhs->kind == Tuple_kind) {
        /* Non-constant-folded tuple */
        rhs_values = rhs->v.Tuple.elts;
        num_values = asdl_seq_LEN(rhs_values);
    } else if (rhs->kind == Constant_kind) {
        /* Since we know the whole value at compile time, might as well call
         * into the runtime to execute. */
        PyObject* value = rhs->v.Constant.value;
        PyObject* result = PyObject_CallMethod(str, "__mod__", "(O)", value);
        if (result == nullptr) {
            /* Bail out. The expression wasn't well-formed. */
            PyErr_Clear();
            return e;
        }
        assert(PyUnicode_CheckExact(result));
        return Str(result, lhs->lineno, lhs->col_offset, c->c_arena);
    } else if (rhs->kind == Num_kind) {
        /* Since we know the whole value at compile time, might as well call
         * into the runtime to execute. */
        PyObject* value = rhs->v.Num.n;
        PyObject* result = PyObject_CallMethod(str, "__mod__", "O", value);
        if (result == nullptr) {
            /* Bail out. The expression wasn't well-formed. */
            PyErr_Clear();
...
static int
compiler_slice(struct compiler *c, slice_ty s, expr_context_ty)
{
    int n = 2;
...
PyAST_Compile(mod_ty mod, const char *filename, PyCompilerFlags *flags,
              PyArena *arena)
{
    return py::compile::PyAST_CompileEx(mod, filename, flags, -1, arena);
}
