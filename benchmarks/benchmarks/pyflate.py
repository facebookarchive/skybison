        ...
        for finish, endbits in bootstrap[1:]:
            if bits:
                for code in range(start, finish):
                    lengthList.append(HuffmanLength(code, bits))
            start, bits = finish, endbits
            if endbits == -1:
                break
        lengthList.sort(key=HuffmanLength._sort_func)
        self.table = lengthList
...
class OrderedHuffmanTable(HuffmanTable):
    def __init__(self, lengths):
        length = len(lengths)
        z = list(zip(range(length), lengths)) + [(length, -1)]
...
def compute_selectors_list(b, huffman_groups):
    ...
    selectors_list = []
    for _i in range(selectors_used):
...
def compute_tables(b, huffman_groups, symbols_in_use):
    groups_lengths = []
    for _j in range(huffman_groups):
        length = b.readbits(5)
        lengths = []
        for _i in range(symbols_in_use):
            if not 0 <= length <= 20:
    ...
    with open(filename, "rb") as input_fp:
        ...
        for _ in range_it:
            ...
            if magic == 0x1F8B:  # GZip
                ...
                raise Exception(
                    "Unknown file magic %x, not a gzip/bzip2 file" % hex(magic)
                )
