...
TEST_F(UnderOsModuleTest, CloseWithBadFdRaisesOsError) {
  ...
  EXPECT_TRUE(
      raised(runBuiltin(UnderOsModule::close, fd_obj), LayoutId::kOSError));
}
...
TEST_F(UnderOsModuleTest, CloseReturnsNone) {
  ...
  EXPECT_TRUE(runBuiltin(UnderOsModule::close, fd_obj).isNoneType());
  EXPECT_TRUE(
      raised(runBuiltin(UnderOsModule::close, fd_obj), LayoutId::kOSError));
}
...
TEST_F(UnderOsModuleTest, FstatSizeWithBadFdRaisesOSError) {
  ...
  Object fd_obj(&scope, SmallInt::fromWord(fd));
  EXPECT_TRUE(
      raised(runBuiltin(UnderOsModule::fstatSize, fd_obj), LayoutId::kOSError));
}
...
TEST_F(UnderOsModuleTest, FstatSizeReturnsSizeOfFile) {
  ...
  ASSERT_GT(fd, 0);
  Object fd_obj(&scope, SmallInt::fromWord(fd));
  EXPECT_TRUE(isIntEqualsWord(runBuiltin(UnderOsModule::fstatSize, fd_obj),
                              std::strlen(contents)));
}
...
TEST_F(UnderOsModuleTest, FtruncateWithBadFdRaisesOSError) {
  ...
  EXPECT_TRUE(raised(runBuiltin(UnderOsModule::ftruncate, fd_obj, size),
                     LayoutId::kOSError));
}
...
TEST_F(UnderOsModuleTest, FtruncateSetsSize) {
  ...
  EXPECT_TRUE(
      runBuiltin(UnderOsModule::ftruncate, fd_obj, size_obj).isNoneType());
}
...
TEST_F(UnderOsModuleTest, IsattyWithBadFdReturnsFalse) {
  ...
  int fd = 99999;
  Object fd_obj(&scope, SmallInt::fromWord(fd));
  EXPECT_EQ(runBuiltin(UnderOsModule::isatty, fd_obj), Bool::falseObj());
}
...
TEST_F(UnderOsModuleTest, IsdirWithFileReturnsFalse) {
  ...
  ASSERT_GE(fd, 0);
  Object fd_obj(&scope, SmallInt::fromWord(fd));
  EXPECT_EQ(runBuiltin(UnderOsModule::isdir, fd_obj), Bool::falseObj());
  EXPECT_EQ(::close(fd), 0);
}
...
TEST_F(UnderOsModuleTest, IsdirWithNonExistentFdRaisesOSError) {
  ...
  Object fd_obj(&scope, SmallInt::fromWord(fd));
  EXPECT_TRUE(
      raised(runBuiltin(UnderOsModule::isdir, fd_obj), LayoutId::kOSError));
}
...
TEST_F(UnderOsModuleTest, IsdirWithDirectoryReturnsTrue) {
  ...
  Object fd_obj(&scope, SmallInt::fromWord(fd));
  EXPECT_EQ(runBuiltin(UnderOsModule::isdir, fd_obj), Bool::trueObj());
}
...
TEST_F(UnderOsModuleTest, LseekWithBadFdRaisesOSError) {
  ...
  EXPECT_TRUE(raised(runBuiltin(UnderOsModule::lseek, fd_obj, offset, whence),
                     LayoutId::kOSError));
}
...
TEST_F(UnderOsModuleTest, LseekChangesPosition) {
  ...
  Object whence(&scope, SmallInt::fromWord(SEEK_SET));
  Object result(&scope,
                runBuiltin(UnderOsModule::lseek, fd_obj, offset_obj, whence));
...
TEST_F(UnderOsModuleTest, OpenWithNonExistentFileRaisesOSError) {
  ...
  EXPECT_TRUE(raised(runBuiltin(UnderOsModule::open, path, flags, mode, dir_fd),
                     LayoutId::kOSError));
}
...
TEST_F(UnderOsModuleTest, OpenReturnsInt) {
  ...
  Object dir_fd(&scope, NoneType::object());
  Object result_obj(&scope, runBuiltin(UnderOsModule::open, path_obj, flags_obj,
                                       mode_obj, dir_fd));
...
TEST_F(UnderOsModuleTest, ReadWithBadFdRaisesOSError) {
  ...
  EXPECT_TRUE(raised(runBuiltin(UnderOsModule::read, fd_obj, size),
                     LayoutId::kOSError));
}
...
TEST_F(UnderOsModuleTest,
  ...
  Object fd_obj(&scope, SmallInt::fromWord(fd));
  Object size(&scope, SmallInt::fromWord(5));
  Object result(&scope, runBuiltin(UnderOsModule::read, fd_obj, size));
...
TEST_F(UnderOsModuleTest, SetNoInheritableWithBadFdRaisesOSError) {
  HandleScope scope(thread_);
  Object fd_obj(&scope, SmallInt::fromWord(-1));
  EXPECT_TRUE(raised(runBuiltin(UnderOsModule::setNoinheritable, fd_obj),
                     LayoutId::kOSError));
}
...
TEST_F(UnderOsModuleTest, SetNoInheritableWithFdSetsNoInheritable) {
  ...
  ASSERT_NO_FATAL_FAILURE(createDummyFdWithContents("hello, world!", &fd));
  Object fd_obj(&scope, SmallInt::fromWord(fd));
  Object result(&scope, runBuiltin(UnderOsModule::setNoinheritable, fd_obj));
