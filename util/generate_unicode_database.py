...
class DatabaseRecord:
    ...
    def __str__(self):
        mirrored = "true" if self.mirrored else "false"
        return f"{{{self.bidirectional:#04x}, {self.category:#04x}, \
{self.combining:#04x}, {self.east_asian_width:#04x}, {mirrored}, \
{self.quick_check:#04x}}}"
...
class Reindex:
    ...
    def __str__(self):
        return f"{{{self.start:#010x}, {self.count}, {self.index}}}"
    ...
    def dump(self, file, trace=False):
        ...
        if self.data:
            ...
            for item in self.data:
                file.write(f'    RawSmallStr::fromCStr("{item}"),\n')
...
// Longest decomposition in Unicode {UNIDATA_VERSION}: U+FDFA
static const int kMaxDecomposition = {MAX_DECOMPOSITION};
...
def write_string_literals(db, trace):
    SmallStrArray("kBidirectionalNames", BIDIRECTIONAL_NAMES).dump(db, trace)
    SmallStrArray("kCategoryNames", CATEGORY_NAMES).dump(db, trace)
    SmallStrArray("kEastAsianWidthNames", EASTASIANWIDTH_NAMES).dump(db, trace)
...
def write_db_coda(db):
    db.write(
        """
int32_t composeCodePoint(int32_t first, int32_t last) {
  int32_t i = first * kTotalLast + last;
  int32_t j = kCompIndex[i >> kCompShift];
...
static int32_t findNFC(const Reindex* nfc, int32_t code_point) {
  for (word index = 0; nfc[index].start != 0; index++) {
    int32_t delta = code_point - nfc[index].start;
    if (delta < 0) {
      return -1;
    }
    if (delta <= nfc[index].count) {
      return delta + nfc[index].index;
    }
  }
  return -1;
}
...
int32_t findNFCFirst(int32_t code_point) {
  return findNFC(kNFCFirst, code_point);
}

int32_t findNFCLast(int32_t code_point) {
  return findNFC(kNFCLast, code_point);
}
...
const UnicodeDatabaseRecord* databaseRecord(int32_t code_point) {
  ...
  int index = kDatabaseIndex1[code_point >> kDatabaseIndexShift];
...
def write_db(trace=False):
    ...
    with open(DB_PATH, "w") as db:
        ...
        write_string_literals(db, trace)
        write_db_coda(db)
