    ...
    def output_templates(self, f):
        ...
        if not parameters:
            ...
            if not new_or_init:
                ...
                parser_code = [normalize_snippet("""
                    Py_ssize_t copy_len = Py_MIN(nargs, %s);
                    for (Py_ssize_t i = 0; i < copy_len; i++) {{
                        argsbuf[i] = PyTuple_GET_ITEM(args, i);
                    }}
                    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, %d, %d, %d, argsbuf);
                    if (!fastargs) {{
                        goto exit;
                    }}
                    """ % (len(converters), min_pos, max_pos, min_kw_only), indent=4)]
        ...
        if self.format_unit == 'O!':
            ...
            return """
                if (!PyObject_TypeCheck({argname}, {subclass_of})) {{{{
                    _PyArg_BadArgument("{{name}}", {displayname}, _PyType_Name({subclass_of}), {argname});
                    goto exit;
                }}}}
                {paramname} = {cast}{argname};
                """.format(argname=argname, paramname=self.name,
