...
{
    run_at_forkers(_posixstate_global->before_forkers, 1);
...
PyOS_AfterFork_Parent(void)
{
    ...
    run_at_forkers(_posixstate_global->after_forkers_parent, 0);
}
...
PyOS_AfterFork_Child(void)
{
    ...
    run_at_forkers(_posixstate_global->after_forkers_child, 0);
}
...
// Facebook: Copy the implementation of _PyType_LookupSpecial to use
// the string in the module state rather than a _Py_Identifier
static PyObject*
fspath_lookup_special(PyObject *self) {
    PyObject *func, *getter;
    PyTypeObject *path_type;
    path_type = Py_TYPE(self);
...
/*[clinic end generated code: output=5398ac75e8e97625 input=cd1187aa85d2312e]*/
{
    ...
    if (register_at_forker(&_posixstate_global->before_forkers, before)) {
        return NULL;
    }
    if (register_at_forker(&_posixstate_global->after_forkers_child, after_in_child)) {
        return NULL;
    }
    if (register_at_forker(&_posixstate_global->after_forkers_parent, after_in_parent)) {
        return NULL;
    }
    Py_RETURN_NONE;
}
...
convert_sched_param(PyObject *param, struct sched_param *res)
{
    ...
    SchedParamType = _posixstate_global->SchedParamType;
...
win32_putenv(PyObject *name, PyObject *value)
{
    ...
    PyObject *unicode = PyUnicode_FromFormat("%U=%V", name, value, "");
    if (unicode == NULL) {
        return NULL;
    }
...
#ifdef USE_XATTRS
static ssize_t
try_getxattr(path_t *path, path_t *attribute, int follow_symlinks,
             void *buffer, size_t size)
{
    ssize_t result;
    Py_BEGIN_ALLOW_THREADS;
    if (path->fd >= 0) {
        result = fgetxattr(path->fd, attribute->narrow, buffer, size);
    } else if (follow_symlinks) {
        result = getxattr(path->narrow, attribute->narrow, buffer, size);
    } else {
        result = lgetxattr(path->narrow, attribute->narrow, buffer, size);
    }
    Py_END_ALLOW_THREADS;
    return result;
}
...
/*[clinic end generated code: output=5f2f44200a43cff2 input=025789491708f7eb]*/
{
    ...
    _PyBytesWriter_Init(&writer);
    buffer = _PyBytesWriter_Alloc(&writer, 128);
    if (buffer == NULL) {
        return NULL;
    }
    result = try_getxattr(path, attribute, follow_symlinks, buffer, 128);
    if (result >= 0) {
        return _PyBytesWriter_Finish(&writer, buffer + result);
    }
    if (errno != ERANGE) {
        goto error;
    }

    buffer = _PyBytesWriter_Resize(&writer, buffer, XATTR_SIZE_MAX);
    if (buffer == NULL) {
        return NULL;
    }
    result = try_getxattr(path, attribute, follow_symlinks, buffer, XATTR_SIZE_MAX);
    if (result >= 0) {
        return _PyBytesWriter_Finish(&writer, buffer + result);
    }
...
/*[clinic end generated code: output=42c5cca9d18068e9 input=4067cdb1b6776c29]*/
{
    ...
    _PyBytesWriter_Init(&writer);
    buffer = _PyBytesWriter_Alloc(&writer, size);
    if (buffer == NULL)
        return NULL;

    result = _PyOS_URandom(buffer, size);
    if (result == -1) {
        _PyBytesWriter_Dealloc(&writer);
        return NULL;
    }
    return _PyBytesWriter_Finish(&writer, buffer + size);
}
...
static PyMethodDef SchedParamType_dunder_new = {
    "__new__", (PyCFunction)os_sched_param, METH_CLASS | METH_VARARGS | METH_KEYWORDS
};
#endif

static PyMethodDef StatResultType_dunder_new = {
    "__new__", (PyCFunction)statresult_new, METH_CLASS | METH_VARARGS
};
...
#ifdef HAVE_FORK
    Py_VISIT(_posixstate(module)->before_forkers);
    Py_VISIT(_posixstate(module)->after_forkers_child);
    Py_VISIT(_posixstate(module)->after_forkers_parent);
#endif
    return 0;
}
...
#ifdef HAVE_FORK
    Py_CLEAR(_posixstate(module)->before_forkers);
    Py_CLEAR(_posixstate(module)->after_forkers_child);
    Py_CLEAR(_posixstate(module)->after_forkers_parent);
#endif
    return 0;
}
...
INITFUNC(void)
{
    ...
    _posixstate(m)->stat_float_times = 1;
...
#ifdef HAVE_FORK
    _posixstate(m)->before_forkers = NULL;
    _posixstate(m)->after_forkers_child = NULL;
    _posixstate(m)->after_forkers_parent = NULL;
#endif
