...
#define localemodulestate(o) ((localemodulestate *)PyModule_GetState(o))
#define localemodulestate_global localemodulestate(PyState_FindModule(&_localemodule))
...
PyLocale_setlocale(PyObject* self, PyObject* args)
{
    ...
    {
        PyErr_SetString(localemodulestate_global->Error, "invalid locale category");
        return NULL;
    }
#endif
    ...
    if (locale) {
        ...
        if (!result) {
            ...
            PyErr_SetString(localemodulestate_global->Error, "unsupported locale setting");
            return NULL;
        }
        ...
        if (!result) {
            PyErr_SetString(localemodulestate_global->Error, "locale query failed");
            return NULL;
        }
        result_object = PyUnicode_DecodeLocale(result, NULL);
    }
...
PyIntl_bindtextdomain(PyObject* self,PyObject*args)
{
    ...
    if (!PyArg_ParseTuple(args, "sO", &domain, &dirname_obj))
        return 0;
    if (!strlen(domain)) {
        PyErr_SetString(localemodulestate_global->Error, "domain must be a non-empty string");
        return 0;
    }
...
static int
localemodule_traverse(PyObject *m, visitproc visit, void *arg) {
    Py_VISIT(localemodulestate(m)->Error);
    return 0;
}

static int
localemodule_clear(PyObject *m) {
    Py_CLEAR(localemodulestate(m)->Error);
    return 0;
}

static void
localemodule_free(void *m) {
    localemodule_clear((PyObject *)m);
}
...
static struct PyModuleDef _localemodule = {
    ...
    "_locale",
    locale__doc__,
    sizeof(localemodulestate),
    PyLocale_Methods,
    NULL,
    localemodule_traverse,
    localemodule_clear,
    localemodule_free,
};
...
PyInit__locale(void)
{
    ...
    m = PyState_FindModule(&_localemodule);
    if (m != NULL) {
        Py_INCREF(m);
        return m;
    }
    ...
    localemodulestate(m)->Error = Error;
