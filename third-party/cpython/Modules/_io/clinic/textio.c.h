...
_io_IncrementalNewlineDecoder___init__(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, 2, 3, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    decoder = fastargs[0];
...
_io_TextIOWrapper___init__(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    Py_ssize_t copy_len = Py_MIN(nargs, 6);
    for (Py_ssize_t i = 0; i < copy_len; i++) {
        argsbuf[i] = PyTuple_GET_ITEM(args, i);
    }
    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, 1, 6, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    buffer = fastargs[0];
...
/*[clinic end generated code: output=fcc555fbe2843acf input=a9049054013a1b77]*/
