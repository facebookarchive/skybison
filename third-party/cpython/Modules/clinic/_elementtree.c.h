...
_elementtree_Element_append(ElementObject *self, PyObject *arg)
{
    ...
    if (!PyObject_TypeCheck(arg, &Element_Type)) {
        _PyArg_BadArgument("append", "argument", _PyType_Name(&Element_Type), arg);
        goto exit;
    }
    subelement = arg;
...
_elementtree_Element_insert(ElementObject *self, PyObject *const *args, Py_ssize_t nargs)
{
    ...
    if (!PyObject_TypeCheck(args[1], &Element_Type)) {
        _PyArg_BadArgument("insert", "argument 2", _PyType_Name(&Element_Type), args[1]);
        goto exit;
    }
    subelement = args[1];
...
_elementtree_Element_remove(ElementObject *self, PyObject *arg)
{
    ...
    if (!PyObject_TypeCheck(arg, &Element_Type)) {
        _PyArg_BadArgument("remove", "argument", _PyType_Name(&Element_Type), arg);
        goto exit;
    }
    subelement = arg;
...
_elementtree_TreeBuilder___init__(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    Py_ssize_t copy_len = Py_MIN(nargs, 5);
    for (Py_ssize_t i = 0; i < copy_len; i++) {
        argsbuf[i] = PyTuple_GET_ITEM(args, i);
    }
    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, 0, 1, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    if (!noptargs) {
...
_elementtree_XMLParser___init__(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, 0, 0, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    if (!noptargs) {
...
/*[clinic end generated code: output=8900ea80a69fe498 input=a9049054013a1b77]*/
