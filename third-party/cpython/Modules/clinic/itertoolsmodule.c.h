...
itertools__grouper(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyObject_TypeCheck(PyTuple_GET_ITEM(args, 0), &groupby_type)) {
        _PyArg_BadArgument("_grouper", "argument 1", _PyType_Name(&groupby_type), PyTuple_GET_ITEM(args, 0));
        goto exit;
    }
...
itertools_combinations(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, 2, 2, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    iterable = fastargs[0];
...
itertools_accumulate(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    Py_ssize_t copy_len = Py_MIN(nargs, 3);
    for (Py_ssize_t i = 0; i < copy_len; i++) {
        argsbuf[i] = PyTuple_GET_ITEM(args, i);
    }
    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, 1, 2, 0, argsbuf);
...
itertools_count(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, 0, 2, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    if (!noptargs) {
...
/*[clinic end generated code: output=fbaf79135735427a input=a9049054013a1b77]*/
