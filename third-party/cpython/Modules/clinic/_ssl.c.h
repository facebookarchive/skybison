...
_ssl__SSLContext__wrap_socket(PySSLContext *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (!_PyArg_ParseStackAndKeywords(args, nargs, kwnames, &_parser,
        _sslmodulestate_global->sock_type, &sock, &server_side, &hostname_obj, &owner, &session)) {
        goto exit;
    }
...
_ssl__SSLContext__wrap_bio(PySSLContext *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (!_PyArg_ParseStackAndKeywords(args, nargs, kwnames, &_parser,
        _sslmodulestate_global->PySSLMemoryBIO_Type, &incoming, _sslmodulestate_global->PySSLMemoryBIO_Type, &outgoing, &server_side, &hostname_obj, &owner, &session)) {
        goto exit;
    }
...
/*[clinic end generated code: output=454df51ba239f30c input=a9049054013a1b77]*/
