...
_ssl__SSLContext__wrap_socket(PySSLContext *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (!PyObject_TypeCheck(args[0], _sslmodulestate_global->sock_type)) {
        _PyArg_BadArgument("_wrap_socket", "argument 'sock'", _PyType_Name(_sslmodulestate_global->sock_type), args[0]);
        goto exit;
    }
    sock = args[0];
...
_ssl__SSLContext__wrap_bio(PySSLContext *self, PyObject *const *args, Py_ssize_t nargs, PyObject *kwnames)
{
    ...
    if (!PyObject_TypeCheck(args[0], _sslmodulestate_global->PySSLMemoryBIO_Type)) {
        _PyArg_BadArgument("_wrap_bio", "argument 'incoming'", _PyType_Name(_sslmodulestate_global->PySSLMemoryBIO_Type), args[0]);
        goto exit;
    }
    incoming = (PySSLMemoryBIO *)args[0];
    if (!PyObject_TypeCheck(args[1], _sslmodulestate_global->PySSLMemoryBIO_Type)) {
        _PyArg_BadArgument("_wrap_bio", "argument 'outgoing'", _PyType_Name(_sslmodulestate_global->PySSLMemoryBIO_Type), args[1]);
        goto exit;
    }
...
/*[clinic end generated code: output=fc12f3be666e12f9 input=a9049054013a1b77]*/
