...
_pickle_Pickler___init__(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    Py_ssize_t copy_len = Py_MIN(nargs, 4);
    for (Py_ssize_t i = 0; i < copy_len; i++) {
        argsbuf[i] = PyTuple_GET_ITEM(args, i);
    }
    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, 1, 4, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    file = fastargs[0];
...
/*[clinic end generated code: output=e16adfda0d11a611 input=a9049054013a1b77]*/
