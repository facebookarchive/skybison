            ...
            PyInterpreterState **interp_p)
{
    ...
    if (_PyCapsule_Init() < 0) {
      status =_PyStatus_ERR("can't init PyCapsule");
    }
    ...
    if (_PySTEntry_Init() < 0) {
      status =_PyStatus_ERR("can't init PySTEntry");
    }
...
new_interpreter(PyThreadState **tstate_p)
{
    ...
    if (_PyCapsule_Init() < 0)
        return _PyStatus_ERR("can't init PyCapsule");

    if (_PySTEntry_Init() < 0) {
        return _PyStatus_ERR("can't init PySTEntry");
    }
