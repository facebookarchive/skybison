...
tb_new(PyTypeObject *type, PyObject *args, PyObject *kwargs)
{
    ...
    fastargs = _PyArg_UnpackKeywords(argsbuf, nargs, kwargs, NULL, &_parser, 4, 4, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    tb_next = fastargs[0];
    if (!PyObject_TypeCheck(fastargs[1], &PyFrame_Type)) {
        _PyArg_BadArgument("TracebackType", "argument 'tb_frame'", _PyType_Name(&PyFrame_Type), fastargs[1]);
        goto exit;
    }
...
/*[clinic end generated code: output=8ff1f8af5f30bfe5 input=a9049054013a1b77]*/
