...
_RAW_MAGIC_NUMBER = marshal.magic_number  # For import.c
...
def _find_module_shim(self, fullname):
    ...
    if loader is None and len(portions):
        _warnings.warn(
            f"Not importing directory {portions[0]}: missing __init__", ImportWarning
        )
    return loader
...
def _compile_bytecode(data, name=None, bytecode_path=None, source_path=None):
    ...
    if isinstance(code, _code_type):
        ...
        raise ImportError(
            f"Non-code object in {bytecode_path!r}", name=name, path=bytecode_path
        )
...
def spec_from_file_location(  # noqa: C901
    ...
    def find_spec(cls, fullname, path=None, target=None):
        ...
        for loader, suffixes in _get_supported_file_loaders():
            if filepath.endswith(tuple(suffixes)):
                spec = _bootstrap.spec_from_loader(
                        ...
                        if hash_based:
                            ...
                            if _imp.check_hash_based_pycs != "never" and (
                                ...
                                source_hash = _imp.source_hash(
                                    _RAW_MAGIC_NUMBER, source_bytes
                                )
                                _validate_hash_pyc(
                            ...
                            _validate_timestamp_pyc(
                                data, source_mtime, st["size"], fullname, exc_details
                            )
...
class SourcelessFileLoader(FileLoader, _LoaderBasics):
    ...
    def get_code(self, fullname):
        ...
        # TODO(T38902048)
        # return _compile_bytecode(
        #    memoryview(data)[16:], name=fullname, bytecode_path=path
        # )
...
class ExtensionFileLoader(FileLoader, _LoaderBasics):
    ...
    def exec_module(self, module):
        ...
        _bootstrap._verbose_message(
            "extension module {!r} executed from {!r}", self.name, self.path
        )
...
class FileFinder:
    ...
    def _get_spec(self, loader_class, fullname, path, smsl, target):
        ...
        return spec_from_file_location(
            fullname, path, loader=loader, submodule_search_locations=smsl
        )
