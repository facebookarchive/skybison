...
import dis
import math
from compiler.consts import CO_FUTURE_ANNOTATIONS, CO_NOFREE
...
class Python37Tests(CompilerTest):
    ...
    def test_future_gen_stop(self):
        code = self.compile(
            "from __future__ import generator_stop", Python37CodeGenerator
        )
        self.assertEqual(
            code.co_flags,
            CO_NOFREE,
            ...
            for outer_instr in self.graph_to_instrs(outer_graph):
                if outer_instr.opname == "LOAD_CONST" and isinstance(
                    ...
                    for instr in self.graph_to_instrs(outer_instr.oparg.graph):
                        if saw_aiter:
                            self.assertEqual(
                                instr.opname == "LOAD_CONST", generator is CodeGenerator
                            )
        ...
        for generator in (Python37CodeGenerator, CodeGenerator):
            ...
            for instr in self.graph_to_instrs(graph):
                if instr.opname == "POP_EXCEPT":
                    self.assertEqual(
                        prev_instr.opname == "END_FINALLY",
                        generator is Python37CodeGenerator,
                        prev_instr.opname,
                    )
                prev_instr = instr
