...
def _formatwarnmsg(msg):
    ...
    try:
        ...
        if fw is not _formatwarning_orig:
            ...
            return fw(
                msg.message, msg.category, msg.filename, msg.lineno, line=msg.line
            )
    return _formatwarnmsg_impl(msg)
...
def _setoption(arg):
    ...
    if lineno:
        try:
            ...
            raise _OptionError(f"invalid lineno {lineno!r}")
    else:
        lineno = 0
...
def _getcategory(category):
    ...
    if re.match("^[a-zA-Z0-9_]+$", category):
        try:
            ...
            raise _OptionError(f"unknown warning category: {category!r}")
    else:
        i = category.rfind(".")
        module = category[:i]
        klass = category[i + 1 :]
        try:
            ...
            raise _OptionError(f"invalid module name: {module!r}")
        try:
            cat = getattr(m, klass)
        except AttributeError:
            raise _OptionError(f"unknown warning category: {category!r}")
    if not issubclass(cat, Warning):
        raise _OptionError(f"invalid warning category: {category}")
    return cat
...
class WarningMessage(object):
    ...
    def __str__(self):
        return (
            ...
            f" filename : {self.filename!r}, lineno : {self.lineno}, "
            f"line : {self.line!r} }}"
        )
...
try:
    from _warnings import filters, _defaultaction, _onceregistry, _filters_mutated
