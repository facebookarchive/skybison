...
class DumpTestV3(unittest.TestCase):
    ...
    def test_dump_with_negative_large_int(self):
        with io.BytesIO() as f:
            self.assertEqual(marshal.dump(-(2 ** 70), f, 3), 15)
            self.assertEqual(
                f.getvalue(),
    ...
    def test_dump_with_complex(self):
        with io.BytesIO() as f:
            self.assertEqual(marshal.dump(complex(1.1, 2.2), f, 3), 17)
            self.assertEqual(
                f.getvalue(),
    ...
    def test_dump_with_tuple(self):
        with io.BytesIO() as f:
            self.assertEqual(marshal.dump((None,) * 300, f, 3), 305),
            self.assertEqual(
                f.getvalue(),
    ...
    def test_dump_with_code(self):
        ...
        with io.BytesIO() as f:
            ...
            self.assertEqual(
                ...
                b"\x04\x00\x00\x00cellr\x00\x00\x00\x00u\x03\x00\x00\x00baz\xf5"
                b"\x06\x00\x00\x00bucket(\x01\x00\x00\x00r\x02\x00\x00\x00(\x01"
                b"\x00\x00\x00r\x01\x00\x00\x00u\x04\x00\x00\x00nameu\x08"
    ...
    def test_dump_with_ref_uses_ref(self):
        ...
        with io.BytesIO() as f:
            self.assertEqual(marshal.dump(container, f, 3), 23)
            self.assertEqual(
                f.getvalue(),
                b"(\x03\x00\x00\x00\xdb\x03\x00\x00\x00TFN"
                b"r\x00\x00\x00\x00r\x00\x00\x00\x00",
            )
...
class DumpsTestV3(unittest.TestCase):
    def test_dumps_with_none_returns_n(self):
        self.assertEqual(marshal.dumps(None, 3), b"N")

    def test_dumps_with_stop_iteration_returns_s(self):
        self.assertEqual(marshal.dumps(StopIteration, 3), b"S")

    def test_dumps_with_ellipsis_returns_dot(self):
        self.assertEqual(marshal.dumps(..., 3), b".")

    def test_dumps_with_false_returns_f(self):
        self.assertEqual(marshal.dumps(False, 3), b"F")
    ...
    def test_dumps_with_float_nan(self):
        self.assertEqual(
            marshal.dumps(float("nan"), 3), b"g\x00\x00\x00\x00\x00\x00\xf8\x7f"
        )

    def test_dumps_with_float_infinity(self):
        self.assertEqual(
            marshal.dumps(float("infinity"), 3), b"g\x00\x00\x00\x00\x00\x00\xf0\x7f"
        )

    def test_dumps_with_float_negative_infinity(self):
        self.assertEqual(
            marshal.dumps(float("-infinity"), 3), b"g\x00\x00\x00\x00\x00\x00\xf0\xff"
        )

    def test_dumps_with_complex(self):
        self.assertEqual(
            marshal.dumps(complex(1.1, 2.2), 3),
            b"y\x9a\x99\x99\x99\x99\x99\xf1?\x9a\x99\x99\x99\x99\x99\x01@",
        )
