...
def namedtuple(typename, field_names, *, verbose=False, rename=False, module=None):
    ...
    for name in [typename] + field_names:
        ...
        if not name.isidentifier():
            raise ValueError(f'Type names and field names must be valid '
                             f'identifiers: {name!r}')
        if _iskeyword(name):
            raise ValueError(f'Type names and field names cannot be a '
                             f'keyword: {name!r}')
    seen = set()
    for name in field_names:
        if name.startswith('_') and not rename:
            raise ValueError(f'Field names cannot start with an underscore: '
                             f'{name!r}')
        if name in seen:
...
class {typename}(tuple):
    ...
    def _make(cls, iterable, new=tuple.__new__, len=len):
        ...
        if len(result) != {num_fields:d}:
            raise TypeError(f'Expected {num_fields:d} arguments, got {{len(result):d}}')
        return result
